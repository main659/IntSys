...
build_noun_lists <- function(doc)
{
  ...
  prsn <- vector("list", length(X))
  loct <- vector("list", length(X))
  org <- vector("list", length(X))
...
db_clustering <- dbscan(dist.matrix, eps = 0.75, minPts = 3)
...
it_train <- itoken(X, tolower, word_tokenizer, progressbar = TRUE)
vocab <- create_vocabulary(it_train)
vectorizer <- vocab_vectorizer(vocab)
dtm_train <- create_dtm(it_train, vectorizer)
#TO-DO: Train word2vec
